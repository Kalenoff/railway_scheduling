import matplotlib.pyplot as plt
import matplotlib.image as mpimg
import re
import os

input_env = """cell((0,0), 0).
cell((0,1), 0).
cell((0,2), 0).
cell((0,3), 0).
cell((0,4), 0).
cell((0,5), 0).
cell((0,6), 0).
cell((0,7), 0).
cell((0,8), 0).
cell((0,9), 0).
cell((0,10), 0).
cell((0,11), 0).
cell((0,12), 0).
cell((0,13), 0).
cell((0,14), 0).
cell((0,15), 0).
cell((0,16), 0).
cell((0,17), 0).
cell((0,18), 0).
cell((0,19), 0).
cell((0,20), 0).
cell((0,21), 0).
cell((0,22), 0).
cell((0,23), 0).
cell((0,24), 0).
cell((0,25), 0).
cell((0,26), 0).
cell((0,27), 0).
cell((0,28), 0).
cell((0,29), 0).

cell((1,0), 0).
cell((1,1), 16386).
cell((1,2), 1025).
cell((1,3), 1025).
cell((1,4), 1025).
cell((1,5), 1025).
cell((1,6), 4608).
cell((1,7), 16386).
cell((1,8), 1025).
cell((1,9), 1025).
cell((1,10), 1025).
cell((1,11), 4608).
cell((1,12), 0).
cell((1,13), 0).
cell((1,14), 0).
cell((1,15), 0).
cell((1,16), 0).
cell((1,17), 0).
cell((1,18), 0).
cell((1,19), 0).
cell((1,20), 0).
cell((1,21), 0).
cell((1,22), 0).
cell((1,23), 0).
cell((1,24), 0).
cell((1,25), 0).
cell((1,26), 0).
cell((1,27), 0).
cell((1,28), 0).
cell((1,29), 0).

cell((2,0), 0).
cell((2,1), 32800).
cell((2,2), 0).
cell((2,3), 0).
cell((2,4), 0).
cell((2,5), 0).
cell((2,6), 32800).
cell((2,7), 32800).
cell((2,8), 0).
cell((2,9), 0).
cell((2,10), 0).
cell((2,11), 32800).
cell((2,12), 0).
cell((2,13), 0).
cell((2,14), 0).
cell((2,15), 0).
cell((2,16), 0).
cell((2,17), 0).
cell((2,18), 0).
cell((2,19), 0).
cell((2,20), 0).
cell((2,21), 0).
cell((2,22), 0).
cell((2,23), 0).
cell((2,24), 0).
cell((2,25), 0).
cell((2,26), 0).
cell((2,27), 0).
cell((2,28), 0).
cell((2,29), 0).

cell((3,0), 0).
cell((3,1), 32800).
cell((3,2), 0).
cell((3,3), 0).
cell((3,4), 0).
cell((3,5), 0).
cell((3,6), 32872).
cell((3,7), 37408).
cell((3,8), 0).
cell((3,9), 0).
cell((3,10), 0).
cell((3,11), 32800).
cell((3,12), 0).
cell((3,13), 0).
cell((3,14), 0).
cell((3,15), 0).
cell((3,16), 0).
cell((3,17), 0).
cell((3,18), 0).
cell((3,19), 0).
cell((3,20), 0).
cell((3,21), 0).
cell((3,22), 0).
cell((3,23), 0).
cell((3,24), 0).
cell((3,25), 0).
cell((3,26), 0).
cell((3,27), 0).
cell((3,28), 0).
cell((3,29), 0).

cell((4,0), 0).
cell((4,1), 32800).
cell((4,2), 0).
cell((4,3), 0).
cell((4,4), 0).
cell((4,5), 0).
cell((4,6), 49186).
cell((4,7), 34864).
cell((4,8), 0).
cell((4,9), 0).
cell((4,10), 0).
cell((4,11), 32800).
cell((4,12), 0).
cell((4,13), 0).
cell((4,14), 0).
cell((4,15), 0).
cell((4,16), 0).
cell((4,17), 0).
cell((4,18), 0).
cell((4,19), 0).
cell((4,20), 0).
cell((4,21), 0).
cell((4,22), 0).
cell((4,23), 0).
cell((4,24), 0).
cell((4,25), 0).
cell((4,26), 0).
cell((4,27), 0).
cell((4,28), 0).
cell((4,29), 0).

cell((5,0), 0).
cell((5,1), 32800).
cell((5,2), 0).
cell((5,3), 0).
cell((5,4), 0).
cell((5,5), 16386).
cell((5,6), 34864).
cell((5,7), 32872).
cell((5,8), 4608).
cell((5,9), 0).
cell((5,10), 0).
cell((5,11), 32800).
cell((5,12), 0).
cell((5,13), 0).
cell((5,14), 0).
cell((5,15), 0).
cell((5,16), 0).
cell((5,17), 0).
cell((5,18), 0).
cell((5,19), 0).
cell((5,20), 0).
cell((5,21), 0).
cell((5,22), 0).
cell((5,23), 0).
cell((5,24), 0).
cell((5,25), 0).
cell((5,26), 0).
cell((5,27), 0).
cell((5,28), 0).
cell((5,29), 0).

cell((6,0), 0).
cell((6,1), 32800).
cell((6,2), 0).
cell((6,3), 0).
cell((6,4), 0).
cell((6,5), 32800).
cell((6,6), 32800).
cell((6,7), 32800).
cell((6,8), 32800).
cell((6,9), 0).
cell((6,10), 0).
cell((6,11), 32800).
cell((6,12), 0).
cell((6,13), 0).
cell((6,14), 0).
cell((6,15), 0).
cell((6,16), 0).
cell((6,17), 0).
cell((6,18), 0).
cell((6,19), 0).
cell((6,20), 0).
cell((6,21), 0).
cell((6,22), 0).
cell((6,23), 0).
cell((6,24), 0).
cell((6,25), 0).
cell((6,26), 0).
cell((6,27), 0).
cell((6,28), 0).
cell((6,29), 0).

cell((7,0), 0).
cell((7,1), 32800).
cell((7,2), 0).
cell((7,3), 0).
cell((7,4), 0).
cell((7,5), 72).
cell((7,6), 37408).
cell((7,7), 49186).
cell((7,8), 2064).
cell((7,9), 0).
cell((7,10), 0).
cell((7,11), 32800).
cell((7,12), 0).
cell((7,13), 0).
cell((7,14), 0).
cell((7,15), 0).
cell((7,16), 0).
cell((7,17), 0).
cell((7,18), 0).
cell((7,19), 0).
cell((7,20), 0).
cell((7,21), 0).
cell((7,22), 0).
cell((7,23), 0).
cell((7,24), 0).
cell((7,25), 0).
cell((7,26), 0).
cell((7,27), 0).
cell((7,28), 0).
cell((7,29), 0).

cell((8,0), 0).
cell((8,1), 32800).
cell((8,2), 0).
cell((8,3), 0).
cell((8,4), 0).
cell((8,5), 0).
cell((8,6), 32872).
cell((8,7), 37408).
cell((8,8), 0).
cell((8,9), 0).
cell((8,10), 0).
cell((8,11), 32800).
cell((8,12), 0).
cell((8,13), 0).
cell((8,14), 0).
cell((8,15), 0).
cell((8,16), 0).
cell((8,17), 0).
cell((8,18), 0).
cell((8,19), 0).
cell((8,20), 0).
cell((8,21), 0).
cell((8,22), 0).
cell((8,23), 0).
cell((8,24), 0).
cell((8,25), 0).
cell((8,26), 0).
cell((8,27), 0).
cell((8,28), 0).
cell((8,29), 0).

cell((9,0), 0).
cell((9,1), 32800).
cell((9,2), 0).
cell((9,3), 0).
cell((9,4), 0).
cell((9,5), 0).
cell((9,6), 49186).
cell((9,7), 34864).
cell((9,8), 0).
cell((9,9), 0).
cell((9,10), 0).
cell((9,11), 32800).
cell((9,12), 0).
cell((9,13), 0).
cell((9,14), 0).
cell((9,15), 0).
cell((9,16), 0).
cell((9,17), 0).
cell((9,18), 0).
cell((9,19), 0).
cell((9,20), 0).
cell((9,21), 0).
cell((9,22), 0).
cell((9,23), 0).
cell((9,24), 0).
cell((9,25), 0).
cell((9,26), 0).
cell((9,27), 0).
cell((9,28), 0).
cell((9,29), 0).

cell((10,0), 0).
cell((10,1), 32800).
cell((10,2), 0).
cell((10,3), 0).
cell((10,4), 0).
cell((10,5), 0).
cell((10,6), 32800).
cell((10,7), 32800).
cell((10,8), 0).
cell((10,9), 0).
cell((10,10), 0).
cell((10,11), 32800).
cell((10,12), 0).
cell((10,13), 0).
cell((10,14), 0).
cell((10,15), 0).
cell((10,16), 0).
cell((10,17), 0).
cell((10,18), 0).
cell((10,19), 0).
cell((10,20), 0).
cell((10,21), 0).
cell((10,22), 0).
cell((10,23), 0).
cell((10,24), 0).
cell((10,25), 0).
cell((10,26), 0).
cell((10,27), 0).
cell((10,28), 0).
cell((10,29), 0).

cell((11,0), 0).
cell((11,1), 72).
cell((11,2), 1025).
cell((11,3), 1025).
cell((11,4), 1025).
cell((11,5), 1025).
cell((11,6), 37408).
cell((11,7), 49186).
cell((11,8), 1025).
cell((11,9), 1025).
cell((11,10), 1025).
cell((11,11), 2064).
cell((11,12), 0).
cell((11,13), 0).
cell((11,14), 0).
cell((11,15), 0).
cell((11,16), 0).
cell((11,17), 0).
cell((11,18), 0).
cell((11,19), 0).
cell((11,20), 0).
cell((11,21), 0).
cell((11,22), 0).
cell((11,23), 0).
cell((11,24), 0).
cell((11,25), 0).
cell((11,26), 0).
cell((11,27), 0).
cell((11,28), 0).
cell((11,29), 0).

cell((12,0), 0).
cell((12,1), 0).
cell((12,2), 0).
cell((12,3), 0).
cell((12,4), 0).
cell((12,5), 16386).
cell((12,6), 34864).
cell((12,7), 32872).
cell((12,8), 4608).
cell((12,9), 0).
cell((12,10), 0).
cell((12,11), 0).
cell((12,12), 0).
cell((12,13), 0).
cell((12,14), 0).
cell((12,15), 0).
cell((12,16), 0).
cell((12,17), 0).
cell((12,18), 0).
cell((12,19), 0).
cell((12,20), 0).
cell((12,21), 0).
cell((12,22), 0).
cell((12,23), 0).
cell((12,24), 0).
cell((12,25), 0).
cell((12,26), 0).
cell((12,27), 0).
cell((12,28), 0).
cell((12,29), 0).

cell((13,0), 0).
cell((13,1), 0).
cell((13,2), 0).
cell((13,3), 0).
cell((13,4), 0).
cell((13,5), 32800).
cell((13,6), 32800).
cell((13,7), 32800).
cell((13,8), 32800).
cell((13,9), 0).
cell((13,10), 0).
cell((13,11), 0).
cell((13,12), 0).
cell((13,13), 0).
cell((13,14), 0).
cell((13,15), 0).
cell((13,16), 0).
cell((13,17), 0).
cell((13,18), 0).
cell((13,19), 0).
cell((13,20), 0).
cell((13,21), 0).
cell((13,22), 0).
cell((13,23), 0).
cell((13,24), 0).
cell((13,25), 0).
cell((13,26), 0).
cell((13,27), 0).
cell((13,28), 0).
cell((13,29), 0).

cell((14,0), 0).
cell((14,1), 0).
cell((14,2), 0).
cell((14,3), 0).
cell((14,4), 0).
cell((14,5), 32800).
cell((14,6), 32800).
cell((14,7), 32800).
cell((14,8), 32800).
cell((14,9), 0).
cell((14,10), 0).
cell((14,11), 0).
cell((14,12), 0).
cell((14,13), 0).
cell((14,14), 0).
cell((14,15), 0).
cell((14,16), 0).
cell((14,17), 0).
cell((14,18), 0).
cell((14,19), 0).
cell((14,20), 0).
cell((14,21), 0).
cell((14,22), 0).
cell((14,23), 0).
cell((14,24), 0).
cell((14,25), 0).
cell((14,26), 0).
cell((14,27), 0).
cell((14,28), 0).
cell((14,29), 0).

cell((15,0), 0).
cell((15,1), 0).
cell((15,2), 0).
cell((15,3), 0).
cell((15,4), 0).
cell((15,5), 32800).
cell((15,6), 32800).
cell((15,7), 32800).
cell((15,8), 32800).
cell((15,9), 0).
cell((15,10), 0).
cell((15,11), 0).
cell((15,12), 0).
cell((15,13), 0).
cell((15,14), 0).
cell((15,15), 0).
cell((15,16), 0).
cell((15,17), 0).
cell((15,18), 0).
cell((15,19), 0).
cell((15,20), 0).
cell((15,21), 0).
cell((15,22), 0).
cell((15,23), 0).
cell((15,24), 0).
cell((15,25), 0).
cell((15,26), 0).
cell((15,27), 0).
cell((15,28), 0).
cell((15,29), 0).

cell((16,0), 0).
cell((16,1), 0).
cell((16,2), 0).
cell((16,3), 0).
cell((16,4), 0).
cell((16,5), 32800).
cell((16,6), 32800).
cell((16,7), 32800).
cell((16,8), 32800).
cell((16,9), 0).
cell((16,10), 0).
cell((16,11), 0).
cell((16,12), 0).
cell((16,13), 0).
cell((16,14), 0).
cell((16,15), 0).
cell((16,16), 0).
cell((16,17), 0).
cell((16,18), 0).
cell((16,19), 0).
cell((16,20), 0).
cell((16,21), 0).
cell((16,22), 0).
cell((16,23), 0).
cell((16,24), 0).
cell((16,25), 0).
cell((16,26), 0).
cell((16,27), 0).
cell((16,28), 0).
cell((16,29), 0).

cell((17,0), 0).
cell((17,1), 0).
cell((17,2), 0).
cell((17,3), 0).
cell((17,4), 0).
cell((17,5), 32800).
cell((17,6), 32800).
cell((17,7), 32800).
cell((17,8), 32800).
cell((17,9), 0).
cell((17,10), 0).
cell((17,11), 0).
cell((17,12), 0).
cell((17,13), 0).
cell((17,14), 0).
cell((17,15), 0).
cell((17,16), 0).
cell((17,17), 0).
cell((17,18), 0).
cell((17,19), 0).
cell((17,20), 0).
cell((17,21), 0).
cell((17,22), 0).
cell((17,23), 0).
cell((17,24), 0).
cell((17,25), 0).
cell((17,26), 0).
cell((17,27), 0).
cell((17,28), 0).
cell((17,29), 0).

cell((18,0), 0).
cell((18,1), 0).
cell((18,2), 0).
cell((18,3), 0).
cell((18,4), 0).
cell((18,5), 32800).
cell((18,6), 32800).
cell((18,7), 32800).
cell((18,8), 32800).
cell((18,9), 0).
cell((18,10), 0).
cell((18,11), 0).
cell((18,12), 0).
cell((18,13), 0).
cell((18,14), 0).
cell((18,15), 0).
cell((18,16), 0).
cell((18,17), 0).
cell((18,18), 0).
cell((18,19), 0).
cell((18,20), 0).
cell((18,21), 0).
cell((18,22), 0).
cell((18,23), 0).
cell((18,24), 0).
cell((18,25), 0).
cell((18,26), 0).
cell((18,27), 0).
cell((18,28), 0).
cell((18,29), 0).

cell((19,0), 0).
cell((19,1), 16386).
cell((19,2), 1025).
cell((19,3), 1025).
cell((19,4), 1025).
cell((19,5), 2064).
cell((19,6), 32800).
cell((19,7), 32800).
cell((19,8), 72).
cell((19,9), 1025).
cell((19,10), 1025).
cell((19,11), 4608).
cell((19,12), 0).
cell((19,13), 0).
cell((19,14), 0).
cell((19,15), 0).
cell((19,16), 0).
cell((19,17), 0).
cell((19,18), 0).
cell((19,19), 0).
cell((19,20), 0).
cell((19,21), 0).
cell((19,22), 0).
cell((19,23), 0).
cell((19,24), 0).
cell((19,25), 0).
cell((19,26), 0).
cell((19,27), 0).
cell((19,28), 0).
cell((19,29), 0).

cell((20,0), 0).
cell((20,1), 32800).
cell((20,2), 0).
cell((20,3), 0).
cell((20,4), 0).
cell((20,5), 0).
cell((20,6), 32800).
cell((20,7), 32800).
cell((20,8), 0).
cell((20,9), 0).
cell((20,10), 0).
cell((20,11), 32800).
cell((20,12), 0).
cell((20,13), 0).
cell((20,14), 0).
cell((20,15), 0).
cell((20,16), 0).
cell((20,17), 0).
cell((20,18), 0).
cell((20,19), 0).
cell((20,20), 0).
cell((20,21), 0).
cell((20,22), 0).
cell((20,23), 0).
cell((20,24), 0).
cell((20,25), 0).
cell((20,26), 0).
cell((20,27), 0).
cell((20,28), 0).
cell((20,29), 0).

cell((21,0), 0).
cell((21,1), 32800).
cell((21,2), 0).
cell((21,3), 0).
cell((21,4), 0).
cell((21,5), 0).
cell((21,6), 32872).
cell((21,7), 37408).
cell((21,8), 0).
cell((21,9), 0).
cell((21,10), 0).
cell((21,11), 32800).
cell((21,12), 0).
cell((21,13), 0).
cell((21,14), 0).
cell((21,15), 0).
cell((21,16), 0).
cell((21,17), 0).
cell((21,18), 0).
cell((21,19), 0).
cell((21,20), 0).
cell((21,21), 0).
cell((21,22), 0).
cell((21,23), 0).
cell((21,24), 0).
cell((21,25), 0).
cell((21,26), 0).
cell((21,27), 0).
cell((21,28), 0).
cell((21,29), 0).

cell((22,0), 0).
cell((22,1), 32800).
cell((22,2), 0).
cell((22,3), 0).
cell((22,4), 0).
cell((22,5), 0).
cell((22,6), 49186).
cell((22,7), 34864).
cell((22,8), 0).
cell((22,9), 0).
cell((22,10), 0).
cell((22,11), 32800).
cell((22,12), 0).
cell((22,13), 0).
cell((22,14), 0).
cell((22,15), 0).
cell((22,16), 0).
cell((22,17), 0).
cell((22,18), 0).
cell((22,19), 0).
cell((22,20), 0).
cell((22,21), 0).
cell((22,22), 0).
cell((22,23), 0).
cell((22,24), 0).
cell((22,25), 0).
cell((22,26), 0).
cell((22,27), 0).
cell((22,28), 0).
cell((22,29), 0).

cell((23,0), 0).
cell((23,1), 32800).
cell((23,2), 0).
cell((23,3), 0).
cell((23,4), 0).
cell((23,5), 0).
cell((23,6), 32800).
cell((23,7), 32800).
cell((23,8), 0).
cell((23,9), 0).
cell((23,10), 0).
cell((23,11), 32800).
cell((23,12), 0).
cell((23,13), 0).
cell((23,14), 0).
cell((23,15), 0).
cell((23,16), 0).
cell((23,17), 0).
cell((23,18), 0).
cell((23,19), 0).
cell((23,20), 0).
cell((23,21), 0).
cell((23,22), 0).
cell((23,23), 0).
cell((23,24), 0).
cell((23,25), 0).
cell((23,26), 0).
cell((23,27), 0).
cell((23,28), 0).
cell((23,29), 0).

cell((24,0), 0).
cell((24,1), 32800).
cell((24,2), 0).
cell((24,3), 0).
cell((24,4), 0).
cell((24,5), 0).
cell((24,6), 32800).
cell((24,7), 32800).
cell((24,8), 0).
cell((24,9), 0).
cell((24,10), 0).
cell((24,11), 32800).
cell((24,12), 0).
cell((24,13), 0).
cell((24,14), 0).
cell((24,15), 0).
cell((24,16), 0).
cell((24,17), 0).
cell((24,18), 0).
cell((24,19), 0).
cell((24,20), 0).
cell((24,21), 0).
cell((24,22), 0).
cell((24,23), 0).
cell((24,24), 0).
cell((24,25), 0).
cell((24,26), 0).
cell((24,27), 0).
cell((24,28), 0).
cell((24,29), 0).

cell((25,0), 0).
cell((25,1), 32800).
cell((25,2), 0).
cell((25,3), 0).
cell((25,4), 0).
cell((25,5), 0).
cell((25,6), 32800).
cell((25,7), 32800).
cell((25,8), 0).
cell((25,9), 0).
cell((25,10), 0).
cell((25,11), 32800).
cell((25,12), 0).
cell((25,13), 0).
cell((25,14), 0).
cell((25,15), 0).
cell((25,16), 0).
cell((25,17), 0).
cell((25,18), 0).
cell((25,19), 0).
cell((25,20), 0).
cell((25,21), 0).
cell((25,22), 0).
cell((25,23), 0).
cell((25,24), 0).
cell((25,25), 0).
cell((25,26), 0).
cell((25,27), 0).
cell((25,28), 0).
cell((25,29), 0).

cell((26,0), 0).
cell((26,1), 32800).
cell((26,2), 0).
cell((26,3), 0).
cell((26,4), 0).
cell((26,5), 0).
cell((26,6), 32872).
cell((26,7), 37408).
cell((26,8), 0).
cell((26,9), 0).
cell((26,10), 0).
cell((26,11), 32800).
cell((26,12), 0).
cell((26,13), 0).
cell((26,14), 0).
cell((26,15), 0).
cell((26,16), 0).
cell((26,17), 0).
cell((26,18), 0).
cell((26,19), 0).
cell((26,20), 0).
cell((26,21), 0).
cell((26,22), 0).
cell((26,23), 0).
cell((26,24), 0).
cell((26,25), 0).
cell((26,26), 0).
cell((26,27), 0).
cell((26,28), 0).
cell((26,29), 0).

cell((27,0), 0).
cell((27,1), 32800).
cell((27,2), 0).
cell((27,3), 0).
cell((27,4), 0).
cell((27,5), 0).
cell((27,6), 49186).
cell((27,7), 34864).
cell((27,8), 0).
cell((27,9), 0).
cell((27,10), 0).
cell((27,11), 32800).
cell((27,12), 0).
cell((27,13), 0).
cell((27,14), 0).
cell((27,15), 0).
cell((27,16), 0).
cell((27,17), 0).
cell((27,18), 0).
cell((27,19), 0).
cell((27,20), 0).
cell((27,21), 0).
cell((27,22), 0).
cell((27,23), 0).
cell((27,24), 0).
cell((27,25), 0).
cell((27,26), 0).
cell((27,27), 0).
cell((27,28), 0).
cell((27,29), 0).

cell((28,0), 0).
cell((28,1), 32800).
cell((28,2), 0).
cell((28,3), 0).
cell((28,4), 0).
cell((28,5), 0).
cell((28,6), 32800).
cell((28,7), 32800).
cell((28,8), 0).
cell((28,9), 0).
cell((28,10), 0).
cell((28,11), 32800).
cell((28,12), 0).
cell((28,13), 0).
cell((28,14), 0).
cell((28,15), 0).
cell((28,16), 0).
cell((28,17), 0).
cell((28,18), 0).
cell((28,19), 0).
cell((28,20), 0).
cell((28,21), 0).
cell((28,22), 0).
cell((28,23), 0).
cell((28,24), 0).
cell((28,25), 0).
cell((28,26), 0).
cell((28,27), 0).
cell((28,28), 0).
cell((28,29), 0).

cell((29,0), 0).
cell((29,1), 72).
cell((29,2), 1025).
cell((29,3), 1025).
cell((29,4), 1025).
cell((29,5), 1025).
cell((29,6), 2064).
cell((29,7), 72).
cell((29,8), 1025).
cell((29,9), 1025).
cell((29,10), 1025).
cell((29,11), 2064).
cell((29,12), 0).
cell((29,13), 0).
cell((29,14), 0).
cell((29,15), 0).
cell((29,16), 0).
cell((29,17), 0).
cell((29,18), 0).
cell((29,19), 0).
cell((29,20), 0).
cell((29,21), 0).
cell((29,22), 0).
cell((29,23), 0).
cell((29,24), 0).
cell((29,25), 0).
cell((29,26), 0).
cell((29,27), 0).
cell((29,28), 0).
cell((29,29), 0).
"""

input_trans = """transition(0,23,636,south,wait,636,south) transition(1,18,397,north,wait,397,north) transition(1,19,397,north,wait,397,north) transition(0,0,185,south,move_forward,215,south) transition(0,3,246,south,turn_right,247,east) transition(0,2,216,east,move_forward,246,south) transition(0,1,215,south,move_forward,216,east) transition(0,5,247,east,move_forward,277,south) transition(0,6,277,south,turn_right,276,west) transition(0,8,306,south,move_forward,336,south) transition(0,7,276,west,move_forward,306,south) transition(0,10,336,south,move_forward,366,south) transition(0,12,366,south,move_forward,396,south) transition(0,15,456,south,move_forward,486,south) transition(0,14,426,south,move_forward,456,south) transition(0,13,396,south,move_forward,426,south) transition(0,17,486,south,move_forward,516,south) transition(0,18,516,south,move_forward,546,south) transition(0,21,606,south,move_forward,636,south) transition(0,20,576,south,move_forward,606,south) transition(0,19,546,south,move_forward,576,south) transition(0,24,636,south,turn_right,637,east) transition(0,27,667,south,move_forward,697,south) transition(0,25,637,east,move_forward,667,south) transition(0,29,697,south,move_forward,727,south) transition(1,2,726,north,move_forward,696,north) transition(1,4,666,north,turn_right,667,east) transition(1,3,696,north,move_forward,666,north) transition(1,6,667,east,move_forward,637,north) transition(1,7,637,north,move_forward,607,north) transition(1,11,547,north,move_forward,517,north) transition(1,10,577,north,move_forward,547,north) transition(1,9,607,north,move_forward,577,north) transition(1,13,517,north,move_forward,487,north) transition(1,14,487,north,move_forward,457,north) transition(1,17,427,north,move_forward,397,north) transition(1,16,457,north,move_forward,427,north) transition(1,20,397,north,move_forward,367,north) transition(1,23,307,north,move_forward,277,north) transition(1,22,337,north,move_forward,307,north) transition(1,21,367,north,move_forward,337,north) transition(1,25,277,north,move_forward,247,north) transition(1,26,247,north,move_forward,217,north) transition(1,29,218,east,move_forward,188,north) transition(1,27,217,north,turn_right,218,east) transition(0,4,246,south,turn_right,247,east) transition(0,9,306,south,move_forward,336,south) transition(0,11,336,south,move_forward,366,south) transition(0,16,456,south,move_forward,486,south) transition(0,22,606,south,move_forward,636,south) transition(0,28,667,south,move_forward,697,south) transition(0,26,637,east,move_forward,667,south) transition(0,30,697,south,move_forward,727,south) transition(1,5,666,north,turn_right,667,east) transition(1,8,637,north,move_forward,607,north) transition(1,12,547,north,move_forward,517,north) transition(1,15,487,north,move_forward,457,north) transition(1,24,307,north,move_forward,277,north) transition(1,30,218,east,move_forward,188,north) transition(1,28,217,north,turn_right,218,east)"""


def dummy_transition_adder(raw_trans: str) -> str:

    transitions_listed = raw_trans.split(' ')
    transitions_listed_sublists = [x.split(',') for x in transitions_listed]
    transition_listed_sorted = sorted(transitions_listed_sublists, key=lambda x: int(x[1]))
    transitions_listed_remerged = [','.join(x) for x in transition_listed_sorted]

    last_transition = transitions_listed_remerged[-1]
    last_transition_listed = last_transition.split(',')

    out_dir = last_transition_listed[-1].strip(')')
    out_pos = last_transition_listed[-2]
    moment = last_transition_listed[1]

    last_transition_listed[2] = out_pos
    last_transition_listed[3] = out_dir
    last_transition_listed[1] = str(int(moment) + 1)
    last_transition_remerged = ','.join(last_transition_listed)

    transitions_listed_remerged.append(last_transition_remerged)

    return_string = ' '.join(transitions_listed_remerged)

    return return_string


def per_train_adder(transitions: str) -> dict:
    """
    Splits transitions by train ID and returns a dictionary where:
    - keys are train IDs (as strings)
    - values are lists of transitions for that train (sorted by moment)
    """
    if not transitions.strip():
        return {}
    
    transitions_listed = [t.strip() for t in transitions.split(" ") if t.strip()]
    train_dict = {}
    
    for transition in transitions_listed:

        match = re.match(r'transition\((\d+)', transition)
        if not match:
            continue
            
        train_id = match.group(1)
        
        if train_id not in train_dict:
            train_dict[train_id] = []
        train_dict[train_id].append(transition)

    for train_id in train_dict:
        # Sort by moment
        train_dict[train_id].sort(key=lambda x: int(re.search(r'transition\(\d+,(\d+)', x).group(1)))
        # Join into single string
        train_trans_str = ' '.join(train_dict[train_id])
        # Apply dummy_transition_adder
        train_dict[train_id] = dummy_transition_adder(train_trans_str)
    
    joined_per_train = [''.join(x) for x in train_dict.values()]
    joined_total = ' '.join(joined_per_train)

    return joined_total


def grid_extractor(env: str):
    env_listed_cleared = [x.replace('\n', '').strip() for x in env.split('.') if x.strip()]
    pos_type_map = {}

    for item in env_listed_cleared:
        match = re.match(r'cell\(\((\d+),\s*(\d+)\),\s*(\d+)\)', item)
        if match:
            y_coord = int(match.group(1))
            x_coord = int(match.group(2))
            value = match.group(3)
            pos_type_map[(y_coord, x_coord)] = value

    return pos_type_map


def onedto2d(flat_pos: int, grid_width: int) -> tuple:

    y = flat_pos // grid_width
    x = flat_pos % grid_width

    return (y, x)


def transition_parser(transitions: str, grid_width: int)->list:

    transitions_extended = per_train_adder(transitions)

    transitions_listed = transitions_extended.split(" ")

    parsed_transition_list = []

    for transition in transitions_listed:

        pattern = r'transition\((\d+),(\d+),(\d+),([^,]+)'
        # pattern = r'transition\(([^,]+),([^,]+),([^,]+),([^,]+)'
        match = re.match(pattern, transition)

        transition_data = {'train': match.group(1),
                           'moment': match.group(2),
                           'pos': onedto2d(int(match.group(3)), grid_width),
                           'dir': match.group(4)}
        
        parsed_transition_list.append(transition_data)

    sorted_data = sorted(parsed_transition_list, key=lambda x: int(x['moment']))

    return sorted_data


def plot_grid_with_images(environment: str, transitions_raw: str, grid_width: int, show1d=False):
    pos_type_map = grid_extractor(environment)
    parsed_transitions = transition_parser(transitions_raw, grid_width)

    # Extract unique train IDs
    train_ids = sorted(set(transition['train'] for transition in parsed_transitions))

    # Assign a color to each train ID
    colors = ['red', 'blue', 'yellow', 'purple', 'orange', 'pink', 'brown', 'gray', 'cyan']
    train_color_map = {train_id: colors[i % len(colors)] for i, train_id in enumerate(train_ids)}

    y_coords = sorted(set(y for y, x in pos_type_map.keys()))
    x_coords = sorted(set(x for y, x in pos_type_map.keys()))
    moments = sorted(set(transition['moment'] for transition in parsed_transitions))

    for moment in moments:
        fig, ax = plt.subplots()
        for x in x_coords:
            for dx in [-0.5, 0.5]:
                ax.axvline(x=x + dx, color='black', linestyle='-')
        for y in y_coords:
            for dy in [-0.5, 0.5]:
                ax.axhline(y=y + dy, color='black', linestyle='-')

        sorted_coords = sorted(pos_type_map.keys())
        for idx, (y, x) in enumerate(sorted_coords):
            value = pos_type_map[(y, x)]
            image_path = os.path.join(r'C:\Users\roman\Documents\Code\ASP\railway_scheduling\python_helpers\lp_to_grid_tiles_backup_2', f"{value}.png")
            if os.path.exists(image_path):
                img = mpimg.imread(image_path)
                ax.imshow(img, extent=[x - 0.5, x + 0.5, y - 0.5, y + 0.5], aspect='auto', origin='lower')
                if show1d:
                    ax.text(x + 0.45, y - 0.45, str(idx), fontsize=8, ha='right', va='top')

        for transition in parsed_transitions:
            if transition['moment'] == moment:
                pos = transition['pos']
                train_id = transition['train']
                color = train_color_map[train_id]
                ax.plot(pos[1], pos[0], marker='o', markersize=10, color=color)

        ax.set_xlim(min(x_coords) - 0.5, max(x_coords) + 0.5)
        ax.set_ylim(max(y_coords) + 0.5, min(y_coords) - 0.5)
        ax.tick_params(axis='both', which='major', labelsize=4)
        ax.set_xticks(x_coords)
        ax.set_yticks(y_coords)
        ax.set_aspect('equal', adjustable='box')
        ax.set_title(f"Moment: {moment}")

        output_path = os.path.join(r"C:\Users\roman\Documents\Code\ASP\railway_scheduling\python_helpers\visualization_outputs", f"moment_{moment}.png")
        plt.savefig(output_path)
        plt.close()


plot_grid_with_images(input_env, input_trans, 30)